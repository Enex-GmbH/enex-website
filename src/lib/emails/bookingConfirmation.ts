...
export async function sendBookingConfirmationEmail(
    ...
    const addonsListForCalendar = addons.length > 0 
        ? addons.map(addon => addon.name).join(", ")
        : "Keine";

    const eventTitle = encodeURIComponent(`ENEX Fahrzeugpflege - ${booking.reference}`);
    ...
    const eventLocation = encodeURIComponent(`${booking.address}, ${booking.postalCode}`);
    ...
    const baseUrl = (process.env.NEXTAUTH_URL || "http://localhost:3000").replace(/^https?:\/\//, "");
